...
class iDataset(data.Dataset):
    ...
    def __init__(self, root,
        ...
        if self.validation:
            ...
            # Build per-task archives from the already-selected 80/20 split.
            if self.train:
                self.archive = []
                domain_i = 0
                for task in self.tasks:
    ...
    def update_coreset(self, coreset_size, seen):
        ...
        state = np.random.get_state()
        np.random.seed(self.seed)
        mapped_targets = np.asarray([self.class_mapping[target] for target in self.targets])
...
class iIMAGENET_R(iDataset):
    ...
    def load(self):
        ...
        if self.train or self.validation:
            split_path = split_dir / 'imagenet-r_train.yaml'
        else:
            split_path = split_dir / 'imagenet-r_test.yaml'
        with split_path.open('r', encoding='utf-8') as split_file:
            data_config = yaml.load(split_file, Loader=yaml.Loader)
        self.data = [self._resolve_path(path) for path in data_config['data']]
    ...
    def _resolve_path(self, img_path):
        ...
        if img_path.is_absolute():
            return str(img_path)
        return str(Path(self.root) / img_path)
    ...
    def parse_archives(self) -> None:
        raise NotImplementedError(
            'ImageNet-R loading uses dataloaders/splits/*.yaml and does not parse archives.'
        )
