...
# Test-Time Augmentation: number of augmented views (1 = no TTA)
TTA_NUM_VIEWS = 5
...
class NormalNN(nn.Module):
    ...
    @staticmethod
    def _tta_augment(images, num_views):
        ...
        for _ in range(num_views - 1):
            ...
            # Small random crop: pad 4px then random crop back to original size
    ...
    def validation(self, dataloader, task_id=None, model=None, task_in=None, task_metric='acc', verbal=True,
        ...
        for i, (input, target, task) in enumerate(dataloader):
            ...
            with torch.no_grad():
                if task_in is None:
                    output = self._forward_tta(model, input, num_views)[:, :self.valid_out_dim]
                    ...
                    if len(target) > 0:
                        if task_global:
                            ...
                            output = self._forward_tta(model, input, num_views)[:, task_in]
