...
def create_args():
    ...
    parser.add_argument('--log_dir', type=str, default="outputs/default",
    ...
    parser.add_argument('--reg_item', type=float, default=1e-4,
                        help="Weight of the L1 key regularization loss (lambda_reg in the paper).")
    parser.add_argument('--triplet_margin', type=float, default=0.002,
                        help="Margin alpha of the key triplet loss.")
    parser.add_argument('--tta_views', type=int, default=1,
                        help="Test-time augmentation views during validation. 1 disables TTA "
                             "(standard protocol; paper baselines report numbers without TTA).")
    ...
    parser.add_argument('--routing_mode', type=str, default='hard',
                        ...
                        help='Inference-time routing strategy. "hard" is the paper setting; '
                             '"topk_uncertainty" is an optional non-paper setting.')
    ...
    parser.add_argument('--routing_query_source', type=str, default='backbone_pass',
                        choices=['backbone_pass', 'shallow_pass', 'shared_block'],
                        help='How to obtain routing queries. "backbone_pass" (paper setting) uses the CLS '
                             'token of a full frozen-backbone pass. "shallow_pass" (optional non-paper setting) uses the CLS '
                             'token after the first --routing_query_depth prompt-free blocks. "shared_block" '
                             'is a compatibility option: its query was input-independent, so it is now a '
                             'compatibility alias of shallow_pass.')
    parser.add_argument('--routing_query_depth', type=int, default=3,
                        help='Number of prompt-free leading blocks used by the shallow_pass query source.')
    ...
    parser.add_argument('--routing_distill_weight', type=float, default=0.0,
                        help='Weight of training-time router distillation from the backbone_pass query to '
                             'the shallow_pass query. 0 disables distillation (paper setting).')
...
def get_args(argv):
    ...
    if config.get('routing_query_source') == 'shared_block':
        print('WARNING: routing_query_source "shared_block" is a compatibility alias because its routing query '
              'was input-independent (the CLS slot before block 0 is a learned constant). '
              'Falling back to "shallow_pass".')
        config['routing_query_source'] = 'shallow_pass'
