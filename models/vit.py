...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None, teacher_q=None):
        B = x.shape[0]
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if prompt is not None:
                if train:
                    p_list, loss, x = prompt.forward(q, i, x, train=True, task_id=task_id, teacher_query=teacher_q)
                    prompt_loss += loss
                else:
