...
class TaskAwarePromptPool(nn.Module):
    def __init__(
        ...
        # Defaults follow the ICASSP 2024 paper setting: hard argmax routing on a
        # frozen-backbone query. Other modes are optional non-paper settings.
        self.routing_mode = routing_config.get('mode', 'hard')
        ...
        self.routing_query_source = routing_config.get('query_source', 'backbone_pass')
        self.routing_query_depth = max(1, int(routing_config.get('query_depth', 3)))
        ...
        self.routing_distill_weight = float(routing_config.get('distill_weight', 0.0))
    ...
    def get_routing_summary(self):
        ...
        return {
            ...
            'routing_query_depth': self.routing_query_depth,
    ...
    def _resolve_query(self, x_query, x_block):
        # The query must come from an image-conditioned encoder pass supplied by
        # the caller (backbone_pass or shallow_pass). The compatibility
        # 'shared_block' option is normalized to shallow_pass in run.py.
        if x_query is None:
            raise ValueError(
                f'Routing query source {self.routing_query_source!r} requires an encoder query, '
                'but none was provided.'
            )
        if self.routing_query_source == 'shallow_pass':
            # Intermediate-layer CLS tokens are not normalized by the backbone's
            # final LayerNorm, so pass them through the trainable query norm.
            return F.normalize(self.query_norm(x_query.detach()), dim=1)
...
class ViTZoo(nn.Module):
    def __init__(self, num_classes=10, pt=False, prompt_flag=False, prompt_param=None, routing_config=None,
                 reg_item=1e-4, triplet_margin=0.002):
        super().__init__()
        self.prompt_flag = prompt_flag
        self.task_id = None
        self.feat = self._build_backbone(pt)
        self.last = nn.Linear(768, num_classes)
        self.prompt = self._build_prompt_module(prompt_param, routing_config, reg_item, triplet_margin)
    ...
    def _build_prompt_module(self, prompt_param, routing_config, reg_item, triplet_margin):
    ...
    def forward(self, x, pen=False, train=False):
        ...
        if self.prompt is not None:
            query_source = self.prompt.routing_query_source
            teacher_query = None
            if query_source == 'backbone_pass':
                # Paper setting: the routing query is the CLS token of a full
                # frozen-backbone pass.
                teacher_query = self._encode_query(x)
                query = teacher_query
            elif query_source == 'shallow_pass':
                query = self._encode_query_shallow(x)
                if self.prompt.should_compute_teacher_query(train=train):
                    teacher_query = self._encode_query(x)
            else:
                raise ValueError(f'Unsupported routing query source: {query_source!r}')
    ...
    def _encode_query_shallow(self, x):
        # Optional non-paper setting: use the first `routing_query_depth`
        # prompt-free blocks instead of the full second backbone pass.
        depth = self.prompt.routing_query_depth
        with torch.no_grad():
            tokens = self.feat.embed_tokens(x)
            for block in self.feat.blocks[:depth]:
                tokens = block(tokens)
        return tokens[:, 0, :]
